...
# ---------------------------------------------------------------------
# Statistic-based loss functions for slowly-varying multidimensional SINN
# ---------------------------------------------------------------------
# This file keeps the same structure as the original SINN loss file:
#
#     StatLoss          : base class that stores the target statistic
#     ACFLoss / CCFLoss : time-correlation losses
#     DensityLoss       : marginal and joint PDF losses
#     Conditional*Loss  : conditional statistics for slowly varying systems
#
# For the slowly varying setting, the key idea is to compare statistics
# conditionally on a slowly varying variable/parameter theta.
# For example, we group samples according to theta and match the conditional
# mean, std, skewness, kurtosis, or conditional PDFs in each theta group.
#
# Expected trajectory shape for most losses:
#
#     data.shape = (trajectory_length, batch_size, dimension)
#
# For conditional losses, the input is usually a tuple:
#
#     (theta, data)
#
# where theta is the conditioning variable and data is the corresponding
# two-dimensional stochastic quantity whose conditional statistics are matched.
# ---------------------------------------------------------------------


class StatLoss(_Loss, metaclass=ABCMeta):
    '''Base class for all statistic-based loss functions.

    This abstract class stores the target statistic computed from empirical
    data, such as ACF, CCF, PDF, or conditional moments. Concrete subclasses
    define how to compute the same statistic from generated trajectories and
    compare it with the stored target.

    In this slowly varying version, some losses also use a conditioning
    variable theta to match statistics within different theta groups.
    '''
    ...
    def __init__(self, target, pointwise_loss='mse_loss', device='cpu', normalize=False,
        ...
        # Store the target statistic. We detach it because it is computed
        # from the reference data and should not receive gradients.
        self._target = target.clone().detach()
        
        # Optional normalization flag used by some KDE-based PDF losses.
    ...
    def normalize_data(data):
        ...
        This is mainly used before KDE evaluation when different variables
        or linear combinations have noticeably different scales.
        '''
        mean = data.mean()
        std = data.std()
    ...
    def acf(x, lags=None, method='fft'):
        # Autocorrelation function for each component of the process.
        # x.shape = (trajectory_length, batch_size, dimension).
        # The result has shape (number_of_lags, dimension).
    ...
    def ccf(x, lags=None, method='fft'):
        # Cross-correlation function between different components.
        # For dimension d, this computes ordered pairs (j, k), j != k.
        # For d = 2, the output contains CCF(q1, q2) and CCF(q2, q1).
    ...
    def gauss_kde2D(x, lower, upper, n, bw):
        # 2D KDE for the joint PDF of x[..., 0] and x[..., 1].
        # This is useful for visualizing or matching the full joint density.
    ...
    def gauss_kde6(x, lower, upper, n, bw=None, normalize=False):
        '''Compute KDEs of selected linear combinations of x[...,0] and x[...,1].

        The name is kept as gauss_kde6 for compatibility with older notebooks.
        In this slowly varying version, the function currently evaluates
        10 linear combinations, including q1 ± q2, q1 ± 2q2, 2q1 ± q2,
        q1 ± 4q2, and 4q1 ± q2.
        '''
    ...
    def gauss_kde(x, lower, upper, n, bw, normalize=False):
        '''Kernel Density Estimation for each component, with optional normalization.'''
        # Output shape: (n_grid, dimension).
...
# ---------------------------------------------------------------------
# Conditional losses for slowly varying systems
# ---------------------------------------------------------------------
# These losses are designed for problems where the statistics of the process
# depend on a slowly varying variable theta. We divide theta into several
# groups and compare the conditional statistics of the generated data with
# those of the reference data in each group.
# ---------------------------------------------------------------------
...
class ConditionalLoss2D(StatLoss):
    ...
    def from_empirical_data(cls, theta, data, **options):
        # theta: conditioning variable, usually the slowly varying parameter.
        # data : two-dimensional quantity whose conditional PDF is matched.
        theta = theta.flatten()
        data = data.view(-1, 2)
        ...
        # Split theta into four groups. The same bounds will be reused
        # when computing the statistic from generated trajectories.
        masks = [
            theta <= bounds[0],
        ...
        return instance
    def forward(self, input):
        # input is a tuple: (theta, generated_data).
        # We apply the same theta groups and compare conditional PDFs.
        theta, data = input
        theta = theta.flatten()
        data = data.view(-1, 2)
...
class ConditionalMeanLoss2D(StatLoss):
    # Match E[data | theta group] for each component of the 2D data.
    @classmethod
    def from_empirical_data(cls, theta, data, bounds=None, scale=10.0, **options):
        ...
        instance.bounds = bounds
        instance.scale = scale   # Store the scale so the same scaling is used during forward().
        return instance
    ...
    def forward(self, input):
        ...
        data = data.view(-1, 2)
        bounds = self.bounds
        scale = self.scale  # Use the same scale as in from_empirical_data().
...
class ConditionalStdLoss2D(StatLoss):
    # Match Std[data | theta group] for each component of the 2D data.
    @classmethod
    def from_empirical_data(cls, theta, data, bounds=None, scale=10.0, **options):
        theta = theta.flatten()
        data = data.view(-1, 2)
...
class ConditionalSkewLoss2D(StatLoss):
    # Match a cube-root-scaled third central moment in each theta group.
    @staticmethod
...
class ConditionalKurtLoss2D(StatLoss):
    # Match a fourth-root-scaled fourth central moment in each theta group.
    @classmethod
    def from_empirical_data(cls, theta, data, bounds=None, scale=10.0, **options):
        theta = theta.flatten()
        data = data.view(-1, 2)
...
def make_loss(stat, data, loss_type=['mse_loss'], **kwargs):
    ...
    def lower_level_loss(a, b):
        # Combine one or more pointwise losses, e.g. ['mse_loss'] or
        # ['mse_loss', 'l1_loss']. This line is kept in the original style.
        return torch.sum([getattr(F, ls)(a, b) for ls in loss_type])
    
    # Special handling for conditional losses.
    # For these losses, data should be given as (theta, target_data).
